...
TextLabeling.prototype.addApi = function(labelingStyle) {
  ...
  var currentNode = this.getCurrentSelectionContainer(),
    instance = TextLabeling.getInstanceWhereCurrentSelectionIsIn(
      currentNode,
      this
    );
  if (instance) {
...
TextLabeling.prototype.addLabeling = function(addCfg) {
  //复制选区内的html结构，文档片段
  var sel = window.getSelection();
  this.currentLabelingStr = addCfg["currentLabelingStr"] = sel.toString(); //dom移除前获取字符
  var wrap_range = window.getSelection().getRangeAt(0),
    cloneHtml = wrap_range.extractContents(),
    wrap_span = document.createElement("span");
  wrap_span.actionCfg = addCfg;
  wrap_span.classList.add("tips-area");
  if (
    addCfg &&
    addCfg.labelingStyle &&
    typeof addCfg.labelingStyle === "object"
  ) {
    let _labelingStyle = addCfg.labelingStyle;
    for (var cssKey in _labelingStyle) {
      if (cssKey === "classNames") {
        wrap_span.classList.add(_labelingStyle[cssKey]);
      } else {
        wrap_span.style[cssKey] = _labelingStyle[cssKey];
      }
    }
  }
  wrap_span.innerHTML =
  ...
  this && this.callback.afterAdd(addCfg);
};
//删除标引
